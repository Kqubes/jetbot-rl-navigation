...
from .jetbot_adr_env_cfg import JetBotADREnvCfg


def define_navigation_markers():
    """Arrows for robot heading and goal direction."""
...
class JetBotADREnv(DirectRLEnv):
    """JetBot navigation environment with adaptive domain randomization."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ADR state
        self.adr_level = cfg.adr_initial_level
        self.adr_window_successes = 0
        self.adr_window_episodes = 0

        self.current_camera_noise_std = 0.0
        self.current_floor_static_friction = 1.0
        self.current_floor_dynamic_friction = 1.0
    ...
    def _apply_adr(self, env_ids: torch.Tensor):
        """Apply wheel-friction and camera-noise randomization."""

        if not self.cfg.adr_enabled:
            return
        ...
        level = float(self.adr_level)

        friction_max = (
            self.cfg.adr_joint_friction_min
            + level
            * (
                self.cfg.adr_joint_friction_max
                - self.cfg.adr_joint_friction_min
            )
        )

        friction = torch.rand(
            (num_resets, len(self.wheel_joint_ids)),
            device=self.device,
        )
        ...
        self.current_camera_noise_std = (
            level * self.cfg.adr_camera_noise_std_max
        )

    def _apply_floor_friction_adr(self):
        """Randomize floor friction based on the ADR level."""
        ...
        static_min = 1.0 - level * (
            1.0 - self.cfg.adr_floor_static_friction_min
        )
        static_max = 1.0 + level * (
            self.cfg.adr_floor_static_friction_max - 1.0
        )

        dynamic_min = 1.0 - level * (
            1.0 - self.cfg.adr_floor_dynamic_friction_min
        )
        dynamic_max = 1.0 + level * (
            self.cfg.adr_floor_dynamic_friction_max - 1.0
        )

        static_friction = (
            static_min
            + torch.rand(1).item() * (static_max - static_min)
        )

        dynamic_friction = (
            dynamic_min
            + torch.rand(1).item() * (dynamic_max - dynamic_min)
        )
        ...
        if not material_prim.IsValid():
            raise RuntimeError(
                "Ground physics material not found at "
                "/World/ground/physicsMaterial"
            )
        ...
        ).Set(dynamic_friction)

        self.current_floor_static_friction = static_friction
        self.current_floor_dynamic_friction = dynamic_friction

    def _update_adr_level(self):
        """Expand or contract ADR difficulty using recent success rate."""

        if not self.cfg.adr_enabled:
            return

        if (
            self.adr_window_episodes
            < self.cfg.adr_update_interval_episodes
        ):
            return

        success_rate = (
            self.adr_window_successes
            / self.adr_window_episodes
        )

        if success_rate >= self.cfg.adr_success_expand_threshold:
            self.adr_level = min(
                self.cfg.adr_max_level,
                self.adr_level + self.cfg.adr_increase_step,
            )

        elif success_rate <= self.cfg.adr_success_contract_threshold:
            self.adr_level = max(
                self.cfg.adr_min_level,
                self.adr_level - self.cfg.adr_decrease_step,
            )

        self._apply_floor_friction_adr()

        self.extras.setdefault("log", {}).update(
            {
                "ADR/level": float(self.adr_level),
                "ADR/window_success_rate": float(success_rate),
                "ADR/floor_static_friction": float(
                    self.current_floor_static_friction
                ),
                "ADR/floor_dynamic_friction": float(
                    self.current_floor_dynamic_friction
                ),
            }
        )
    ...
    def _get_observations(self):
        ...
        if (
            self.cfg.adr_enabled
            and self.current_camera_noise_std > 0.0
        ):
            noise = (
                torch.randn_like(camera_data)
                * self.current_camera_noise_std
            )

            camera_data = torch.clamp(
                camera_data + noise,
                0.0,
                1.0,
            )
    ...
    def _get_dones(self):
        ...
        self.adr_window_successes += reached.sum().item()
        self.adr_window_episodes += completed.sum().item()

        self._update_adr_level()
